...
class WindowAttention(nn.Module):
    ...
    参数：
        ...
        qkv_bias (bool, 可选)：如果为 True，则为查询、键、值添加可学习的偏差。默认值：True
        attn_drop (float, 可选): 注意力权重的 Dropout 比率。默认值：0.0
        proj_drop (float, 可选): 输出的丢包率。默认值：0.0
    """
...
def swin_tiny_patch4_window7_224(pretrained=False, num_classes: int = 1000, **kwargs):
    ...
    if pretrained:
        model_state = torch.load("./model/pretrained/swin_tiny_patch4_window7_224.pth", map_location="cpu")["model"]
        model.load_state_dict(model_state, strict=False)
        model.patch_embed.proj = nn.Conv2d(3, 96, kernel_size=(4, 4), stride=(4, 4))
    return model
